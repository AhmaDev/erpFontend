    ...
    fetch() {
      ...
      this.$http.get("masterSheet/" + this.$route.params.id).then((res) => {
        ...
        for (let i = 0; i < this.mastersheet.students.length; i++) {
          ...
          const avg =
            this.getStudentAvg(student, "try1") < 50
              ? this.getStudentAvg(student, "try2")
              : this.getStudentAvg(student, "try1");
          const successTry =
            this.getStudentAvg(student, "try1") < 50 ? "دور ثاني" : "دور اول";
          this.students.push({
            student: student,
            avg: avg,
